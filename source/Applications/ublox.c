...
static void ubloxSetMode(void) {
    ...
    ubloxWriteU1(0x05);            // mask LSB (fixMode, dyn)
...
unsigned char ubloxPublish(void) {
    ...
    if (ubloxData.class == UBLOX_NAV_CLASS && ubloxData.id == UBLOX_NAV_TIMEUTC && (ubloxData.payload.timeutc.valid & 0x04)) {
